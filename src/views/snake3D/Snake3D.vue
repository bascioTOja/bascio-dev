...
camera.lookAt(
  new THREE.Vector3(
    (Board.tileAmount * Board.tileSize) / 2,
    0,
    (Board.tileAmount * Board.tileSize) / 2
  )
);
...
function getNewSnake() {
  return new Snake(
    3,
    new Vector(
      Math.round(Board.tileAmount * 0.25),
      Math.round(Board.tileAmount / 2)
    ),
    directionMap.right
  );
}
...
function update(dt) {
  ...
  if (
    ...
    snake.checkWallCollision(Board.tileAmount, Board.tileAmount)
  ) {
    restartGame();
  }
...
function render() {
  ...
  ); // TODO: To remove after remove orbitControls
  update(delta);
